...
const MODES: { id: RouteMode; label: string; desc: string; Icon: React.ElementType }[] = [
  ...
  { id: "min_interchange",label: "Fewest Changes",  desc: "Minimum line switches · stay comfortable", Icon: RefreshCw },
];

interface Props {
  onRouteFound?: (route: RouteResponse | null) => void;
}

export default function RoutePlanner({ onRouteFound }: Props) {
  ...
  const handleSwap = () => {
    ...
    setRouteData(null); setError(null);
    onRouteFound?.(null);
  };
  ...
  const handleRoute = async () => {
    ...
    try {
      ...
      if (data.error) {
        ...
        // Auto-scroll to map after short delay
        setTimeout(() => {
    ...
    <div className="rounded-3xl border border-border/70 bg-card/40 backdrop-blur-sm shadow-elevated p-6 sm:p-8 space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3">
        {MODES.map(({ id, label, desc, Icon }) => (
          <button key={id}
            onClick={() => { setMode(id); setRouteData(null); setError(null); onRouteFound?.(null); }}
            className={cn(
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="space-y-3">
          <StationCombobox
            value={start}
            onChange={(v) => { setStart(v); setRouteData(null); setError(null); onRouteFound?.(null); }}
            stations={stations} loading={stationsLoading}
            placeholder="Choose origin station" accentClass="text-primary"
          />
          ...
          <StationCombobox
            value={end}
            onChange={(v) => { setEnd(v); setRouteData(null); setError(null); onRouteFound?.(null); }}
            stations={stations} loading={stationsLoading}
            placeholder="Choose destination station" accentClass="text-accent"
          />
          ...
          {/* Map hint — shown after route found */}
          {routeData?.route && (
            <p className="text-[11px] text-primary/70 text-center animate-fade-in-up">
              ↓ Route highlighted on the map below
            </p>
          )}
        </div>

        {/* Results */}
        <RouteResults loading={loading} route={routeData} start={start} end={end} mode={mode} />
      </div>
    </div>
  );
}